...
export default function ComfortSettings() {
  ...
  useEffect(() => {
    try { const next = { ...DEFAULTS, ...JSON.parse(localStorage.getItem("snoozelet-comfort") || "{}") }; setPreferences(next); applyPreferences(next); }
  ...
  return <div className="comfort-settings">
    <button className="comfort-trigger" type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-controls="comfort-panel">Comfort</button>
    {open ? <section className="comfort-panel" id="comfort-panel" aria-label="Accessibility and comfort settings">
      ...
      <label>Theme<select value={preferences.theme} onChange={(event) => update("theme", event.target.value)}><option value="night">Night</option><option value="light">Light</option><option value="oled">OLED black</option></select></label>
      <label>Text size<select value={preferences.fontSize} onChange={(event) => update("fontSize", event.target.value)}><option value="normal">Normal</option><option value="large">Large</option><option value="xlarge">Extra large</option></select></label>

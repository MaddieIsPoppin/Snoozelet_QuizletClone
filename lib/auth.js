...
import { isHostedEnvironment, isHostedWithoutDatabase, queryOne, queryRun } from "@/lib/db";
...
export async function createUser({ username, password }) {
  ...
  if (cleanedUsername.length > 100) {
    throw new Error("Username must be 100 characters or fewer");
  }
...
export async function createSession(userId) {
  ...
  await queryRun("DELETE FROM sessions WHERE expires_at <= ?", [createdAt.toISOString()]);
  ...
  store.set(SESSION_COOKIE, id, {
    httpOnly: true,
    sameSite: "lax",
    secure: process.env.NODE_ENV === "production" && isHostedEnvironment(),
    path: "/",
    expires: expiresAt
  });
}

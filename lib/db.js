...
export async function updateResourceLink({ resourceId, userId, title, url, type, description }) {
  const cleanTitle = String(title || "").trim();
  if (!cleanTitle) throw new Error("Resource title is required");
  let parsed; try { parsed = new URL(String(url)); } catch { throw new Error("Enter a valid resource URL"); }
  if (!["http:","https:"].includes(parsed.protocol)) throw new Error("Resource links must use http or https");
  return queryRun("UPDATE resource_links SET title=?,url=?,type=?,description=? WHERE id=? AND user_id=?", [cleanTitle.slice(0,120), parsed.href, String(type || "website").slice(0,30), String(description || "").trim().slice(0,300), toInt(resourceId), toInt(userId)]);
}

...
const FORCE_LOCAL = process.env.SNOOZELET_DATABASE_MODE === "local";
const ACTIVE_REMOTE_DB_URL = HOSTED_ENV || !FORCE_LOCAL ? REMOTE_DB_URL : null;
const ACTIVE_REMOTE_DB_TOKEN = HOSTED_ENV || !FORCE_LOCAL ? REMOTE_DB_TOKEN : null;
...
function getClient() {
  ...
  if (!client) {
    if (!ACTIVE_REMOTE_DB_URL) {
      fs.mkdirSync(path.dirname(LOCAL_DB_PATH), { recursive: true });
    }

    client = createClient({
      url: ACTIVE_REMOTE_DB_URL || `file:${LOCAL_DB_PATH}`,
      authToken: ACTIVE_REMOTE_DB_TOKEN
    });
  }

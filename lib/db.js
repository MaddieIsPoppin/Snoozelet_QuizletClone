...
function xpForReview(mode, correct) {
  if (!correct) {
    return 0;
  }
...
export async function recordReview(input) {
  ...
  if (wasCorrect) {
    repetitions += 1;
    streak += 1;
    ease = Math.min(3.2, ease + (review.mode === "typed" ? 0.08 : 0.04));

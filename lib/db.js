...
const LOCAL_DB_PATH = path.resolve(process.env.STUDY_DB_PATH || DEFAULT_DB_PATH);
...
export function getDatabaseInfo() {
  return { mode: "local", path: LOCAL_DB_PATH };
}
...
async function migrate() {
  ...
  await createAutomaticBackup("startup");
  await db.batch(
    [
...
export async function getDecks(userId) {
  ...
  const rows = await queryAll(
    ...
    LEFT JOIN subjects subj ON subj.id = f.subject_id
...
async function createAutomaticBackup(reason = "automatic") {
  ...
  backupPromise = (async () => {
    ...
    const backupDir = path.join(path.dirname(LOCAL_DB_PATH), "backups");
    fs.mkdirSync(backupDir, { recursive: true });
    const date = new Date().toISOString().slice(0, 10);
    const destination = path.join(backupDir, `study-${date}-${reason}.sqlite`);
    if (!fs.existsSync(destination)) {
      const escaped = destination.replaceAll("'", "''");
      await getClient().execute(`VACUUM INTO '${escaped}'`);
    }
    return destination;
...
export async function createLocalBackup(reason = "manual") {
  await ensureReady();
  const stamp = new Date().toISOString().replaceAll(":", "-").replaceAll(".", "-");
  const backupDir = path.join(path.dirname(LOCAL_DB_PATH), "backups");
  fs.mkdirSync(backupDir, { recursive: true });
  const destination = path.join(backupDir, `study-${stamp}-${reason}.sqlite`);
  await getClient().execute(`VACUUM INTO '${destination.replaceAll("'", "''")}'`);
  return destination;
}
...
export async function updateSubject({ subjectId, name, description, userId }) {
  const clean = String(name || "").trim();
  if (!clean) throw new Error("Folder name is required");
  return queryRun("UPDATE subjects SET name=?,description=?,updated_at=? WHERE id=? AND user_id=?", [clean.slice(0,100),String(description||"").trim().slice(0,500),nowIso(),toInt(subjectId),toInt(userId)]);
}

export async function deleteSubject({ subjectId, userId }) {
  return queryRun("DELETE FROM subjects WHERE id=? AND user_id=?", [toInt(subjectId),toInt(userId)]);
}
...
export async function createResourceLink({ userId, subjectId, folderId, title, url, type, description }) {
  ...
  if (subject && !(await queryOne("SELECT id FROM subjects WHERE id=? AND user_id=?", [subject, toInt(userId)]))) throw new Error("Module not found");
  const ownedFolder = folder ? await queryOne("SELECT id,subject_id FROM deck_folders WHERE id=? AND user_id=?", [folder, toInt(userId)]) : null;
  if (folder && !ownedFolder) throw new Error("Study Unit not found");
  if (folder && subject && String(ownedFolder.subject_id) !== String(subject)) throw new Error("Choose a Study Unit inside the selected Module");
  if (await queryOne("SELECT id FROM resource_links WHERE user_id=? AND url=?", [toInt(userId), parsed.href])) throw new Error("This exact resource link is already saved");
...
export async function getResourceLinks(userId) {
  return queryAll(`SELECT r.*, s.name AS subject_name, f.name AS folder_name
    FROM resource_links r
    LEFT JOIN subjects s ON s.id = r.subject_id AND s.user_id = r.user_id
    LEFT JOIN deck_folders f ON f.id = r.folder_id AND f.user_id = r.user_id
    WHERE r.user_id = ? ORDER BY r.created_at DESC`, [toInt(userId)]);
}
...
export async function updateResourceLink({ resourceId, userId, subjectId, folderId, title, url, type, description }) {
  const cleanTitle = String(title || "").trim();
  if (!cleanTitle) throw new Error("Resource title is required");
  let parsed; try { parsed = new URL(String(url)); } catch { throw new Error("Enter a valid resource URL"); }
  if (!["http:","https:"].includes(parsed.protocol)) throw new Error("Resource links must use http or https");
  const subject = subjectId ? toInt(subjectId) : null;
  const folder = folderId ? toInt(folderId) : null;
  if (!subject && !folder) throw new Error("Choose a Module or Study Unit");
  if (subject && !(await queryOne("SELECT id FROM subjects WHERE id=? AND user_id=?", [subject, toInt(userId)]))) throw new Error("Module not found");
  const ownedFolder = folder ? await queryOne("SELECT id,subject_id FROM deck_folders WHERE id=? AND user_id=?", [folder, toInt(userId)]) : null;
  if (folder && !ownedFolder) throw new Error("Study Unit not found");
  if (folder && subject && String(ownedFolder.subject_id) !== String(subject)) throw new Error("Choose a Study Unit inside the selected Module");
  if (await queryOne("SELECT id FROM resource_links WHERE user_id=? AND url=? AND id<>?", [toInt(userId), parsed.href, toInt(resourceId)])) throw new Error("This exact resource link is already saved");
  return queryRun("UPDATE resource_links SET subject_id=?,folder_id=?,title=?,url=?,type=?,description=? WHERE id=? AND user_id=?", [subject, folder, cleanTitle.slice(0,120), parsed.href, String(type || "website").slice(0,30), String(description || "").trim().slice(0,300), toInt(resourceId), toInt(userId)]);
}

export async function getNotes(userId) {
  return queryAll(`SELECT n.*, s.name AS subject_name, f.name AS folder_name, d.title AS deck_title
    FROM notes n LEFT JOIN subjects s ON s.id=n.subject_id LEFT JOIN deck_folders f ON f.id=n.folder_id
    LEFT JOIN decks d ON d.id=n.deck_id WHERE n.user_id=? ORDER BY n.updated_at DESC`, [toInt(userId)]);
}

async function ownedNoteContext({ userId, subjectId, folderId, deckId }) {
  const owner = toInt(userId);
  const subject = subjectId ? toInt(subjectId) : null;
  const folder = folderId ? toInt(folderId) : null;
  const deck = deckId ? toInt(deckId) : null;
  if (subject && !(await queryOne("SELECT id FROM subjects WHERE id=? AND user_id=?", [subject, owner]))) throw new Error("Module not found");
  if (folder && !(await queryOne("SELECT id FROM deck_folders WHERE id=? AND user_id=?", [folder, owner]))) throw new Error("Study Unit not found");
  if (deck && !(await queryOne("SELECT id FROM decks WHERE id=? AND user_id=?", [deck, owner]))) throw new Error("Deck not found");
  return { owner, subject, folder, deck };
}

export async function createNote({ userId, subjectId, folderId, deckId, title, content }) {
  const cleanTitle = String(title || "").trim();
  if (!cleanTitle) throw new Error("Note title is required");
  const context = await ownedNoteContext({ userId, subjectId, folderId, deckId });
  const timestamp = nowIso();
  const result = await queryRun("INSERT INTO notes(user_id,subject_id,folder_id,deck_id,title,content,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)", [context.owner, context.subject, context.folder, context.deck, cleanTitle.slice(0,160), String(content || "").slice(0,100000), timestamp, timestamp]);
  return Number(result.lastInsertRowid);
}

export async function updateNote({ noteId, userId, subjectId, folderId, deckId, title, content }) {
  const cleanTitle = String(title || "").trim();
  if (!cleanTitle) throw new Error("Note title is required");
  const context = await ownedNoteContext({ userId, subjectId, folderId, deckId });
  return queryRun("UPDATE notes SET subject_id=?,folder_id=?,deck_id=?,title=?,content=?,updated_at=? WHERE id=? AND user_id=?", [context.subject, context.folder, context.deck, cleanTitle.slice(0,160), String(content || "").slice(0,100000), nowIso(), toInt(noteId), context.owner]);
}

export async function deleteNote({ noteId, userId }) {
  return queryRun("DELETE FROM notes WHERE id=? AND user_id=?", [toInt(noteId), toInt(userId)]);
}
...
export async function createDeckFolder({ name, userId, subjectId = null }) {
  ...
  const ownerId = toInt(userId);
  const moduleId = subjectId ? toInt(subjectId) : null;
  if (moduleId && !(await queryOne("SELECT id FROM subjects WHERE id = ? AND user_id = ?", [moduleId, ownerId]))) {
    throw new Error("Module not found");
  }
  const duplicate = await queryOne(
    "SELECT id FROM deck_folders WHERE user_id = ? AND name = ? COLLATE NOCASE",
    [ownerId, cleanedName]
  );
  if (duplicate) {
    if (moduleId && !(await queryOne("SELECT id FROM deck_folders WHERE user_id = ? AND subject_id = ? AND name = ? COLLATE NOCASE", [ownerId, moduleId, cleanedName]))) {
      throw new Error("A Study Unit with that name already exists elsewhere. Add the Module code to make the name unique.");
    }
    throw new Error("A Study Unit with that name already exists");
  }
  const timestamp = nowIso();
  const result = await queryRun(
    "INSERT INTO deck_folders (user_id, name, subject_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    [ownerId, cleanedName, moduleId, timestamp, timestamp]
  );
  return Number(result.lastInsertRowid);
}
...
export async function getBackupData(userId) {
  ...
  return {
    ...
    version: 4,
    exportedAt: nowIso(),
    subjects: await queryAll("SELECT * FROM subjects WHERE user_id=? ORDER BY id", [owner]),
    studyUnits: await queryAll("SELECT * FROM deck_folders WHERE user_id=? ORDER BY id", [owner]),
    resources: await getResourceLinks(owner),
    ...
    decks: await Promise.all(decks.map(async (deck) => ({
      ...
      cards: await queryAll(`SELECT c.*, s.ease, s.interval_days, s.repetitions, s.lapses, s.due_at, s.last_reviewed_at,
        s.correct_count, s.incorrect_count, s.streak, s.best_streak, s.weak FROM cards c
        LEFT JOIN study_stats s ON s.card_id=c.id WHERE c.deck_id=? ORDER BY c.id`, [deck.id]),
    }))),
    reviews: await queryAll("SELECT r.* FROM review_logs r JOIN decks d ON d.id=r.deck_id WHERE d.user_id=? ORDER BY r.id", [owner]),
    progress: await queryOne("SELECT * FROM user_progress WHERE user_id=?", [owner]),
    goals: await queryAll("SELECT * FROM learning_goals WHERE user_id=? ORDER BY id", [owner]),
    combinedDecks: await getDeckGroups(owner),
  };
}

function cleanDeckIds(deckIds) {
  return [...new Set((deckIds || []).map(toInt).filter((id) => Number.isInteger(id) && id > 0))];
}

export async function getCombinedStudySnapshot(deckIds, userId) {
  const ids = cleanDeckIds(deckIds);
  if (!ids.length) return { decks: [], cards: [] };
  const placeholders = ids.map(() => "?").join(",");
  const ownedDecks = await queryAll(`SELECT d.*, f.name AS folder_name, s.name AS subject_name
    FROM decks d LEFT JOIN deck_folders f ON f.id=d.folder_id LEFT JOIN subjects s ON s.id=f.subject_id
    WHERE d.user_id=? AND d.id IN (${placeholders}) ORDER BY d.title COLLATE NOCASE`, [toInt(userId), ...ids]);
  const ownedIds = ownedDecks.map((deck) => Number(deck.id));
  if (!ownedIds.length) return { decks: [], cards: [] };
  const ownedPlaceholders = ownedIds.map(() => "?").join(",");
  const cards = await queryAll(`SELECT c.id,c.deck_id,c.term,c.definition,COALESCE(c.hint,'') AS hint,
    c.image_url AS imageUrl,c.image_public_id AS imagePublicId,c.image_alt AS imageAlt,c.created_at,c.updated_at,
    st.ease,st.interval_days,st.repetitions,st.lapses,st.due_at,st.last_reviewed_at,st.correct_count,st.incorrect_count,
    st.streak,st.best_streak,st.weak,d.title AS source_deck_title
    FROM cards c JOIN decks d ON d.id=c.deck_id JOIN study_stats st ON st.card_id=c.id
    WHERE d.user_id=? AND c.deck_id IN (${ownedPlaceholders}) ORDER BY c.id`, [toInt(userId), ...ownedIds]);
  const now = nowIso();
  return { decks: ownedDecks, cards: cards.map((card) => { const attempts=card.correct_count+card.incorrect_count; return {...card,due:card.due_at<=now,weak:Boolean(card.weak),attempts,accuracy:attempts?Math.round(card.correct_count/attempts*100):0}; }) };
}

export async function getDeckGroups(userId) {
  const groups = await queryAll(`SELECT g.*,COUNT(DISTINCT i.deck_id) AS deck_count,COUNT(DISTINCT c.id) AS card_count
    FROM deck_groups g LEFT JOIN deck_group_items i ON i.group_id=g.id LEFT JOIN decks d ON d.id=i.deck_id AND d.user_id=g.user_id
    LEFT JOIN cards c ON c.deck_id=d.id WHERE g.user_id=? GROUP BY g.id ORDER BY g.updated_at DESC`, [toInt(userId)]);
  if (!groups.length) return [];
  const items = await queryAll(`SELECT i.group_id,d.id,d.title FROM deck_group_items i JOIN deck_groups g ON g.id=i.group_id
    JOIN decks d ON d.id=i.deck_id AND d.user_id=g.user_id WHERE g.user_id=? ORDER BY d.title COLLATE NOCASE`, [toInt(userId)]);
  return groups.map((group) => ({...group,decks:items.filter((item)=>Number(item.group_id)===Number(group.id))}));
}

export async function createDeckGroup({ userId, name, deckIds }) {
  await ensureReady();
  const cleanName=String(name||"").trim(); const ids=cleanDeckIds(deckIds);
  if (!cleanName) throw new Error("Enter a name for the combined deck");
  if (ids.length<2) throw new Error("Select at least two decks");
  const placeholders=ids.map(()=>"?").join(",");
  const owned=await queryAll(`SELECT id FROM decks WHERE user_id=? AND id IN (${placeholders})`,[toInt(userId),...ids]);
  if (owned.length!==ids.length) throw new Error("One or more selected decks are unavailable");
  const transaction=await getClient().transaction("write");
  try { const stamp=nowIso(); const result=await transaction.execute({sql:"INSERT INTO deck_groups(user_id,name,created_at,updated_at) VALUES(?,?,?,?)",args:[toInt(userId),cleanName,stamp,stamp]}); const groupId=Number(result.lastInsertRowid); for(const id of ids) await transaction.execute({sql:"INSERT INTO deck_group_items(group_id,deck_id) VALUES(?,?)",args:[groupId,id]}); await transaction.commit(); return groupId; }
  catch(error){await transaction.rollback();throw error;} finally{transaction.close();}
}

export async function updateDeckGroup({ userId, groupId, name, deckIds }) {
  await ensureReady();
  const cleanName=String(name||"").trim(); const ids=cleanDeckIds(deckIds); if(!cleanName)throw new Error("Enter a name"); if(ids.length<2)throw new Error("Choose at least two decks");
  const owned=await queryOne("SELECT id FROM deck_groups WHERE id=? AND user_id=?",[toInt(groupId),toInt(userId)]); if(!owned)throw new Error("Combined deck not found");
  const placeholders=ids.map(()=>"?").join(","); const decks=await queryAll(`SELECT id FROM decks WHERE user_id=? AND id IN (${placeholders})`,[toInt(userId),...ids]); if(decks.length!==ids.length)throw new Error("A selected deck is unavailable");
  const transaction=await getClient().transaction("write"); try{await transaction.execute({sql:"UPDATE deck_groups SET name=?,updated_at=? WHERE id=? AND user_id=?",args:[cleanName,nowIso(),toInt(groupId),toInt(userId)]});await transaction.execute({sql:"DELETE FROM deck_group_items WHERE group_id=?",args:[toInt(groupId)]});for(const id of ids)await transaction.execute({sql:"INSERT INTO deck_group_items(group_id,deck_id) VALUES(?,?)",args:[toInt(groupId),id]});await transaction.commit();}catch(error){await transaction.rollback();throw error;}finally{transaction.close();}
}

export async function deleteDeckGroup({ userId, groupId }) { return queryRun("DELETE FROM deck_groups WHERE id=? AND user_id=?",[toInt(groupId),toInt(userId)]); }

export async function updateDeckFolder({ folderId, name, userId }) {
  const clean = String(name || "").trim();
  if (!clean) throw new Error("Folder name is required");
  return queryRun("UPDATE deck_folders SET name=?,updated_at=? WHERE id=? AND user_id=?", [clean.slice(0,80),nowIso(),toInt(folderId),toInt(userId)]);
}

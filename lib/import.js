...
export function parseSmartPaste(input) {
  ...
  for (const rawLine of lines) {
    ...
    const meta = line.match(/^(Subject|Folder|Study Unit|Set):\s*(.+)$/i);
    if (meta) { const key = /subject/i.test(meta[1]) ? "subject" : /set/i.test(meta[1]) ? "set" : "folder"; metadata[key] = meta[2].trim(); recognizedBlocks = true; continue; }

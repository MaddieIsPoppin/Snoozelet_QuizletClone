const migrations = [
  ...
  {
    version: 10,
    name: "long form study notes",
    async up(transaction) {
      await transaction.execute(`CREATE TABLE IF NOT EXISTS notes (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      ...
      await transaction.execute("CREATE INDEX IF NOT EXISTS idx_notes_user_updated ON notes(user_id, updated_at DESC)");
      await transaction.execute("CREATE INDEX IF NOT EXISTS idx_notes_subject ON notes(subject_id, user_id)");
      await transaction.execute("CREATE INDEX IF NOT EXISTS idx_notes_folder ON notes(folder_id, user_id)");
      await transaction.execute("CREATE INDEX IF NOT EXISTS idx_notes_deck ON notes(deck_id, user_id)");
    },
  },
  {
    version: 11,
    name: "saved combined deck groups",
    async up(transaction) {
      await transaction.execute(`CREATE TABLE IF NOT EXISTS deck_groups (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
        name TEXT NOT NULL,
        created_at TEXT NOT NULL,
      ...
      await transaction.execute(`CREATE TABLE IF NOT EXISTS deck_group_items (
        group_id INTEGER NOT NULL REFERENCES deck_groups(id) ON DELETE CASCADE,
        deck_id INTEGER NOT NULL REFERENCES decks(id) ON DELETE CASCADE,
        PRIMARY KEY(group_id, deck_id)
      )`);
      await transaction.execute("CREATE INDEX IF NOT EXISTS idx_deck_groups_user_updated ON deck_groups(user_id, updated_at DESC)");
      await transaction.execute("CREATE INDEX IF NOT EXISTS idx_deck_group_items_deck ON deck_group_items(deck_id, group_id)");
    },
  },
];

...
export async function syncLocalUserToCloud(userId) {
  ...
  try {
    ...
    const [subjects, folders, decks, cards, stats, reviews, progress, goals, resources, notes] = await Promise.all([
      ...
      queryAll("SELECT * FROM notes WHERE user_id = ? ORDER BY id", [userId]),
    ]);

    const transaction = await remote.transaction("write");
    try {
      await transaction.execute({ sql: "DELETE FROM notes WHERE user_id = ?", args: [cloudUserId] });
      ...
      for (const row of notes) await transaction.execute({ sql: "INSERT INTO notes (user_id, subject_id, folder_id, deck_id, title, content, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", args: [cloudUserId, row.subject_id ? subjectIds.get(Number(row.subject_id)) : null, row.folder_id ? folderIds.get(Number(row.folder_id)) : null, row.deck_id ? deckIds.get(Number(row.deck_id)) : null, row.title, row.content, row.created_at, row.updated_at] });
